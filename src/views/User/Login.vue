<template>
  <div class="flex flex-col max-w-md mx-auto rounded-md p-6 sm:p-10 bg-gray-50 shadow-2xl">
    <div class="mb-6 text-center">
      <h1 class="my-3 text-4xl font-bold text-info">用户登录</h1>
    </div>
    <div class="space-y-8">
      ...
      <div class="space-y-2">
        还没有账号？<router-link to="/user/reg" class="text-info">立刻注册</router-link>
        <button @click="login" class="btn px-8 btn-block btn-info">登录</button>
...
<script setup>
import { userLogin } from '../../plugins/axios'
...
const login = () => {
  userLogin(account.value, password.value).then((resp) => {
    if (resp.code === 200) {
